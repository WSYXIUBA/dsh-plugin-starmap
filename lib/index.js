...
function enumerateProfiles(profilesRoot) {
    const out = [];
    let entries = [];
    try {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            const depSpecs = new Map();
            if (pkg?.dependencies && typeof pkg.dependencies === "object") {
                for (const [k, v] of Object.entries(pkg.dependencies))
                    depSpecs.set(k, String(v));
            }
...
function categorize(moduleName, authoritative) {
    ...
    if (authoritative.has(moduleName))
        return "第三方插件";
    // Desktop shell builtins.
    if (moduleName === "dsh-plugin-desktop" || moduleName === "dsh-community-market" || moduleName === "dsh-market") {
        return "核心基础设施";
    }
    if (moduleName.startsWith("dsh-plugin-desktop/"))
        return "核心基础设施";
    if (/^(dsh-|cordis-)/.test(short))
        return "第三方插件";
    return "其他";
}
/* ── keep only plugin-shaped packages (official + third-party DSH plugins) ── */
const CORE_LIBS = new Set(["cordis", "cosmokit", "schemastery"]);
function isPluginShape(name, authoritative) {
    if (name.startsWith("@deepseek-ai/"))
        return true;
    if (CORE_LIBS.has(name))
        return true;
    if (authoritative.has(name))
        return true;
    if (name === "dsh-plugin-desktop" || name === "dsh-community-market" || name === "dsh-market")
        return true;
    if (name.startsWith("dsh-plugin-desktop/"))
        return true;
    if (name.startsWith("dsh-") || name.startsWith("cordis-"))
        return true;
    if (name.startsWith("@")) {
        const base = name.split("/").pop() || "";
        if (base.startsWith("dsh-") || base.startsWith("cordis-"))
            return true;
    }
    return false;
}
/* ── locate profile root ── */
function findProfilesRoot() {
...
function scanAllPackages(profilesRoot, profiles) {
    const pkgs = new Map();
    ...
    for (const layer of layers) {
        ...
        let entries = [];
        try {
            entries = readdirSync(layer);
        }
        catch {
            continue;
        }
        for (const entry of entries) {
            if (entry.startsWith(".") || entry === ".pnpm")
                continue;
            if (entry.startsWith("@")) {
...
function resolveExportsPath(v) {
    if (typeof v === "string")
        return v;
    if (v && typeof v === "object") {
        const obj = v;
        if (typeof obj.default !== "undefined")
            return resolveExportsPath(obj.default);
        if (typeof obj.import !== "undefined")
            return resolveExportsPath(obj.import);
        if (typeof obj.require !== "undefined")
            return resolveExportsPath(obj.require);
    }
    return null;
}
function extractInjectServices(scanned) {
    const pkg = scanned.pkg;
    const candidates = new Set();
    if (typeof pkg?.main === "string")
        candidates.add(pkg.main);
    ...
    if (dsh)
        candidates.add(dsh);
    for (const fallback of INJECT_ENTRY_FALLBACKS)
    ...
    for (const rel of candidates) {
        ...
        try {
            if (!existsSync(file) || !statSync(file).isFile())
                continue;
            if (statSync(file).size > ENTRY_READ_CAP)
                continue;
...
function buildGraph(hostCtx) {
    ...
    const installSpec = new Map();
    const ownerProfiles = new Map();
    for (const p of profiles) {
        for (const b of p.bundles) {
            if (!ownerProfiles.has(b))
                ownerProfiles.set(b, []);
            ownerProfiles.get(b).push(p.name);
        }
        for (const [depName, spec] of p.depSpecs) {
            installSpec.set(depName, spec);
            if (!ownerProfiles.has(depName))
                ownerProfiles.set(depName, []);
            if (!ownerProfiles.get(depName).includes(p.name))
                ownerProfiles.get(depName).push(p.name);
        }
    }
    ...
    try {
        ...
        const packageNameOf = (moduleName) => {
        ...
        for (const e of entries) {
            ...
            if (!mn)
                continue;
            const phase = (e.fiberPhase === undefined ? null : String(e.fiberPhase));
            const entry = { enabled: e.enabled !== false, phase };
            stateMap.set(mn, entry);
            const pkgName = packageNameOf(mn);
            if (pkgName !== mn && !stateMap.has(pkgName))
                stateMap.set(pkgName, entry);
        }
    }
    catch { /* ignore */ }
    ...
    for (const [name, { pkg }] of pkgs) {
        if (!isPluginShape(name, authoritative))
            continue;
        const nodeId = name;
        if (known.has(nodeId))
            continue;
    ...
    for (const [name, { pkg }] of pkgs) {
        ...
        for (const mod of clientInject) {
            if (typeof mod === "string" && known.has(mod))
                addLink(name, mod, "client");
        }
    }
    ...
    for (const [name, scanned] of pkgs) {
        ...
        for (const svc of extractInjectServices(scanned)) {
            if (!serviceConsumers.has(svc))
                serviceConsumers.set(svc, []);
    ...
    for (const [svc, consumers] of serviceConsumers) {
        ...
        for (const consumer of consumers)
            addLink(consumer, hubId, "service");
    }
    ...
    const referenced = new Set([...authoritative, ...stateMap.keys()]);
    const adj = new Map();
    for (const [name, { pkg }] of pkgs) {
        const targets = [];
        ...
        if (Array.isArray(clientInject)) {
            for (const mod of clientInject) {
                if (typeof mod === "string" && pkgs.has(mod))
                    targets.push(mod);
            }
        }
...
let graphCache = null;
function cacheKey(profilesRoot) {
    const parts = [];
    let entries = [];
    try {
        entries = readdirSync(profilesRoot);
    ...
    for (const entry of entries) {
        ...
        const pkgPath = join(dir, "package.json");
        try {
            parts.push(`${entry}/pkg:${statSync(pkgPath).mtimeMs}`);
        }
        catch { /* ignore */ }
        const layer = join(dir, "node_modules");
        try {
            parts.push(`${entry}/nm:${statSync(layer).mtimeMs}`);
        }
        catch { /* ignore */ }
    }
    try {
        parts.push(`shared/nm:${statSync(join(profilesRoot, "node_modules")).mtimeMs}`);
    }
    catch { /* ignore */ }
    return parts.join("|");
}
...
function migrateOldSettings() {
    ...
    if (existsSync(join(newDir, "settings.json")))
        return;
    try {
...
export function apply(ctx) {
    ctx.inject(["webServer", "loader", "pluginInventory"], (hostCtx) => {
        hostCtx.effect(() => {
                ...
                handler: async (request, response) => {
                    try {
                        ...
                        if (body.bgColor !== undefined) {
                            if (body.bgColor === "auto" || /^#[0-9a-fA-F]{6}$/.test(body.bgColor))
                                s.bgColor = body.bgColor;
                        }
                        if (typeof body.bgOpacity === "number")
                            s.bgOpacity = Math.max(0, Math.min(1, body.bgOpacity));
                        if (typeof body.blur === "number")
                            s.blur = Math.max(0, Math.min(40, body.blur));
