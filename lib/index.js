...
function categorize(moduleName) {
    ...
    if (moduleName === "dshmarket" || moduleName === "dsh-plugin-constellation")
        return "第三方插件";
...
function isPluginShape(name) {
    if (name.startsWith("@deepseek-ai/"))
        return true;
    if (CORE_LIBS.has(name))
        return true;
    if (["dshmarket", "dsh-plugin-constellation", "dsh-plugin-desktop", "dsh-community-market", "dsh-market"].includes(name))
        return true;
    ...
    if (name.startsWith("@")) {
        ...
        if (name.includes("hindsight") || name.includes("coding-agents"))
            return true;
    }
    return false;
}
...
function scanAllPackages(profilesRoot) {
...
function buildGraph(hostCtx) {
    ...
    try {
        ...
        for (const e of entries) {
            ...
            if (mn)
                stateMap.set(mn, e.enabled !== false);
        }
    }
    catch { /* ignore */ }
